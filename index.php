...
if (isset($_SESSION['user_id'])) {
    ...
    $stmt = $pdo->prepare("SELECT first_name FROM customers WHERE customer_id = :user_id");
    ...
    $userImagePath = "Images/{$userId}.png"; // Ścieżka do obrazu użytkownika
    
    // Sprawdzenie, czy plik istnieje, w przeciwnym razie załaduj domyślne zdjęcie
    if (!file_exists($userImagePath)) {
        $userImagePath = "Images/users/default.png"; // Domyślny obraz
    }
    echo '<div class="user-info">';
    echo '<img src="' . $userImagePath . '" class="user-icon" alt="User Icon">'; // Dynamiczne ładowanie obrazu
    echo '<span> Hello, ' . htmlspecialchars($userName) . '</span>';
    echo '</div>';
}
